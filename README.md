...
The commands above are entry points. The pack includes 25 skills total — 24 workflow skills plus the `using-agent-skills` meta-skill. Each skill is a structured workflow with steps, verification gates, and anti-rationalization tables. You can also reference any skill directly.
...
| [gauntlet-loop](skills/gauntlet-loop/SKILL.md) | Reference-backed builder/critic loop with blind MATCH/NOT MATCH verdicts and proof receipts | The user says gauntlet, loop until it wins, or requests an independent comparison gate |
